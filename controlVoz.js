...
let esperandoAlexa = true; // Estado inicial, esperando que se diga "Alexa"
let usuario = "Luis";
...
recognition.onresult = function(event) {
  ...
  if (esperandoAlexa && transcript.includes('alexa')) {
      esperandoAlexa = false;
      document.getElementById('status').textContent = 'Di un comando.';
      console.log('Alexa activada, esperando comando.');
  } else if (!esperandoAlexa) {
      procesarComando(transcript);
      esperandoAlexa = true; // Restablece para esperar "Alexa" de nuevo
  }
};
...
function procesarComando(comando) {
  if (actualizarEstado(comando)) {
      ...
      document.getElementById('status').textContent = 'Comando reconocido: ' + comando + '. Di "Alexa" para seguir.';
  } else {
      document.getElementById('status').textContent = 'Comando no reconocido. Di "Alexa" para intentar de nuevo.';
  }
}
